/**
 * Tests for dual-language origin tracking and federation filtering.
 */
...
describe("shouldPublishToSolid", () => {
    ...
    it("returns true for dual-origin links", () => {
        ...
        assert.equal(shouldPublishToSolid("hash456", getOrigin), true);
    });

    it("returns false for solid-origin links (prevents echo loop)", () => {
        const getOrigin = (key: string): string | null => {
            if (key === "link-origin/hash789") return "solid";
            return null;
        };
        assert.equal(shouldPublishToSolid("hash789", getOrigin), false);
    });
    ...
    it("constructs correct storage key for lookup", () => {
        ...
        shouldPublishToSolid("myLinkHash", getOrigin);
